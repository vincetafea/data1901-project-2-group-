...
#Filtering out the signed cols with multiple etn
#Regex for matching up to and including the first (S) only
...
#merged$Signed %>%
#merged$F = arrange(desc(Freq))

## Works below but only alphabetical
#merged <- sort(merged$Signed, decreasing=T)




#merged <- filter(merged, (grepl("^(.*?)\\(S\\)", Signed)))


##merged <- merged[order(merged$Signed, decreasing = TRUE),]

#TODO bugged -- trying to filter out empty entries
#merged <- filter(merged, !(merged$Signed == ('')))
...
ggplot(merged, aes(x = long, y = lat)) + geom_point()
