...
en$Signum <- str_match(en$`!/R Engelsk översättning`, regex("(?:^(\\S+.){2}.)"))
en <- en[-c(1), ]
col_order <- c("Signum", "!/R Engelsk översättning", "english_translations")
en <- en[, col_order]
...
merged <- fuzzy_inner_join(merged, en, match_fun = str_detect, by = c("Signum" = "Signum"))
...
## Plots
### Lat and long of top 5 carvers
```{r}
#TODO fix scales
scatter1 <- ggplot(top_5_df, aes(x = long, y = lat, colour = Signed)) + 
  geom_point() +
  scale_x_discrete(name = "Longitude", breaks = NULL) + 
  scale_y_discrete(name = "Latitude", breaks = 80)
  #scale_y_continuous(name = "Longitude", breaks = seq(0, 1000, by=100), limits=c(0,1000))
scatter1
```
...
```{r}
scatter2 <- ggplot(top_5_df, aes(x = Signed, fill = Cross_Num)) + geom_bar()
scatter2
